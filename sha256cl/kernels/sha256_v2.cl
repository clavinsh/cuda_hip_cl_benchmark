...
void sha256_v2(__constant uchar *input, uint length, uint *hash)
{
	uint w[64];
...
// hešu vērtību debug vajadzībām
void print_hex_string(__constant uint *data, size_t length)
{
						   ...
						   __global atomic_int *cracked_idx)
{
    ...
    sha256_v2(my_password, pw_size, hash);

    // debugošanas vajadzībām
    if(idx == 0)
    {
        printf("offsets: %d\n", offsets[idx]);
        printf("pw_size: %d\n", pw_size);
        printf("%.10s\n",my_password); // OpenCL neļauj dinamiski noteikt string garumu ar *, tāpēc jāvada skaitlis 

        print_hex_string(target_hash, 8);
